"""Access code management services for Kwikset Smart Locks.

Registers integration-level services for creating, editing, disabling,
enabling, and deleting access codes on Kwikset smart locks.

Services accept a device_id (HA device registry ID) to target a specific
lock, then resolve it to the correct coordinator for API calls.
...
_SERVICE_HANDLERS: list[tuple[str, vol.Schema, Any]] = [
    (
...
def async_setup_services(hass: HomeAssistant) -> None:
    """Register access code management services.
    ...
    for service_name, schema, handler in _SERVICE_HANDLERS:
    ...
    LOGGER.debug("Access code services registered")


@callback
def async_unload_services(hass: HomeAssistant) -> None:
    """Unregister access code management services.

    Should only be called when the last config entry is unloaded.
    """
    for service_name, _, _ in _SERVICE_HANDLERS:
        hass.services.async_remove(DOMAIN, service_name)

    LOGGER.debug("Access code services unregistered")
